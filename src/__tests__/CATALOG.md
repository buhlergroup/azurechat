...
| ID | Target file | Case title | Type | Preconditions/mocks | Steps | Expected outcome |
|---|---|---|---|---|---|---|
| chat-page.unit.history-budget.001 | history-budget.ts | Token estimation is deterministic: chars/4 rounded up, plus reasoning text, plus a flat 1000 per image counted by COUNT not URL length; a tool row's persisted JSON is counted via its content | unit | — | Estimate the same row 25 times; compare a blob ref against a 500 KB data URL | One distinct value; image cost independent of URL length |
| chat-page.unit.history-budget.002 | history-budget.ts | `splitIntoTurns` opens a turn at every user row and keeps its tool/assistant rows with it; rows before the first user row form a trimmable preamble | unit | — | Split mixed rows | Turn boundaries on user rows; `isPreamble` only for the leading block; per-turn token totals |
| chat-page.unit.history-budget.003 | history-budget.ts | Nothing is trimmed at or under budget; an existing summary counts towards it | unit | — | Plan at, just under, and just over budget | `trimmed:false` at exactly budget; `true` one token over |
| chat-page.unit.history-budget.004 | history-budget.ts | An over-budget thread is trimmed in ONE block to ≤ 60 % of budget, cut on a turn boundary, without overshooting, reserving room for the replacement summary | unit | — | Plan 20 000 tokens against a 10 000 budget | `dropped ++ kept` reconstructs the input; first kept row is a user row; putting one turn back breaks the target; watermark is the newest dropped row |
| chat-page.unit.history-budget.005 | history-budget.ts | Hysteresis: after a trim, adding several small turns does not trim again, and it takes many turns to trim a second time | unit | — | Trim, then append small turns repeatedly | No further trim for >10 appended turns |
| chat-page.unit.history-budget.006 | history-budget.ts | The newest `MIN_KEPT_TURNS` (2) turns are never trimmed; a thread of only protected turns reports `targetUnreachable` instead of trimming | unit | — | Plan huge turns against a tiny budget | ≥ 2 turns kept verbatim; `trimmed:false` + `targetUnreachable:true` when nothing is droppable |
...
| chat-page.unit.history-budget.009 | history-budget.ts | `resolveHistoryTrimTargetRatio` defaults to 0.6 and honours only values strictly between 0 and 1 | unit | — | Resolve with 0.5, 1, 0, junk | 1 and 0 rejected (they would remove the hysteresis / the history) |
...
| chat-page.unit.history-budget.011 | history-budget.ts | A trim lands on 60 % of the EFFECTIVE budget, not of the configured one | unit | — | Configure 900 000, guard down to 76 800 with a 128k window, then plan | `plan.budget` 76 800, `plan.target` 46 080, kept ≤ 46 080 |
...
| chat-page.unit.compaction-part.002 | compaction-part.ts | The copy for all three states, and token labels that round (184k, 96k) and never render junk | unit | — | Format each state and a NaN/negative token count | "Compacting older messages…" / "Compacted 12 older turns into a summary (184k → 96k tokens)" / "Trimmed 3 older turns (no summary, feature off)"; singular kept singular |
...
| chat-page.unit.compaction-notice.003 | compaction-notice.tsx | Trimmed without a summary says so and offers no toggle (negative) | unit | same | Render `summarised:false` | "Trimmed 3 older turns (no summary, feature off)" |
...
| chat-page.unit.history-summary-service.001 | history-summary-service.ts | `HISTORY_SUMMARY_ENABLED` is honoured only as exactly `"true"`; the deployment resolves `HISTORY_SUMMARY_DEPLOYMENT_NAME` → the thread's own model → terra → luna → mini → undefined, and a Claude/Foundry thread falls back rather than 404ing | unit | env stubbed per case; MODEL_CONFIGS mocked (it reads deployment names at module load) | Call both resolvers for sol/luna/claude/foundry/unknown | `"TRUE"`/`"1"`/`"yes"` are all off; sol thread → sol deployment; claude/foundry → terra |
| chat-page.unit.history-summary-service.006 | history-summary-service.ts | The summariser is not billed to the user: the module reaches neither the metrics nor the budget service, and the thread's model is threaded through from the compaction record | unit | same | Scan the module source; record a compaction for a sol thread | No `chat-metrics`/`budget-service`/`reportPromptTokens`/`recordUsage`; summariser called with the sol deployment |

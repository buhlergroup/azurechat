...
| chat-page.unit.history-budget.012 | history-budget.ts | A trim that cannot help is not taken: declined when the floor it cannot remove (protected turns + summary allowance + static prefix) is over target, and declined when the replacement summary would cost as much as the turns it replaces; `estimatedTokensAfter` is never ≥ `estimatedTokensBefore` on a trimmed plan | unit | — | Plan the 15k-paste shape, the small-turn shape, and a sweep over reserves × budgets × shapes | `skipReason` "cannot-reach-target" / "no-reduction", nothing dropped, and the invariant holds across the sweep |
| chat-page.unit.history-budget.013 | history-budget.ts | The provider's real `inputTokens` for the previous request decides WHETHER to trim; the estimator still decides how much to drop, and is the fallback for a thread with no usage yet | unit | — | Plan with `measuredTokensBefore` over budget while the estimate is under it, then without it, then with junk | `triggerSource` "measured"/"estimated"; 0/-1/NaN ignored |
